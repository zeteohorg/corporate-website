import type { ProductTranslation } from '../types';

export const en: ProductTranslation = {
	title: 'Results You Can Achieve',
	subtitle: '',
	description:
		'✅ See Where Time Disappears\nIdentify which processes create bottlenecks, when delays happen, and where workflow breaks down—all with objective data.\n\n✅ Eliminate Inefficiencies Fast\nUse real movement data to optimize process flow, remove bottlenecks, and establish SOPs based on what actually works.\n\n✅ Maximize Your Current Team\nIncrease output without new hires by optimizing how your existing workforce moves and operates.\n\n✅ Deploy for a Fraction of Traditional Costs\nGet enterprise-level insights without enterprise budgets—no million-yen systems, no months-long installations.'
};

export const ja: ProductTranslation = {
	title: '得られるベネフィット',
	subtitle: '',
	description:
		'✅ 無駄な時間の「見える化」\nどの工程にボトルネックがあるのか、遅延はいつ起きているのか、ワークフローがどこで破綻しているのかを、客観的なデータで全て把握できます。\n\n✅ 非効率性を迅速に排除\n実際の人の動きのデータに基づいて、ワークフローを再設計し、ボトルネックを取り除き、本当に機能する作業標準（SOP）を確立します。\n\n✅ 既存チームの最大限の活用\n従業員の動きと作業方法を最適化することで、新たな人材を採用することなく生産性を向上させます。\n\n✅ 従来の数分の一のコストで導入\n何百万もするシステムも、数か月にわたる設置工事も不要。大企業レベルのインサイトを、中小企業の予算で手に入れられます。'
};
